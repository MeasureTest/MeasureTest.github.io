    ...
    function ja(Z) {
        ...
        function La(a, d, f, p, k) {
            ...
            var s = oa(a, e.slice(3, 6)),
                ...
                e = oa(a, e.slice(9,
                    12)),
                n = g[0] + s[0] + z[0] + e[0]; - 4 == n || 4 == n ? g = !1 : (n = g[1] + s[1] + z[1] + e[1], g = -4 == n || 4 == n ? !1 : 4 != g[2] + s[2] + z[2] + e[2]);
            if (g) {
                g = d.vertices;
                s = g[0] + g[3] + g[6] + g[9];
                z = g[1] + g[4] + g[7] + g[10];
                ...
                if (d.level < c.level) {
                    ...
                    n = 0.5;
                    if (d.x == s || d.y == s) n = 1 - m.tileResolution / (m.tileResolution + l);
                    var x = 1 - n,
                        e = [],
                        t = n,
                        y = n,
                    ...
                    if (l < m.tileResolution)
                        if (d.x ==
                            s && d.y != s) {
                            if (I = y = 0.5, "d" == d.side || "u" == d.side) A = C = 0.5
                        } else d.x != s && d.y == s && (H = t = 0.5, "l" == d.side || "r" == d.side) && (A = C = 0.5);
                    z <= m.tileResolution && (d.x == s && (t = 0, H = 1, "l" == d.side || "r" == d.side) && (C = 0, A = 1), d.y == s && (y = 0, I = 1, "d" == d.side || "u" == d.side) && (C = 0, A = 1));
                    ...
                    e.push(l);
                    d.x == s && z <= m.tileResolution || (l = [g[0] * t +
                        g[3] * H, g[1] * n + g[4] * x, g[2] * C + g[5] * A, g[3], g[4], g[5], g[3] * n + g[6] * x, g[4] * y + g[7] * I, g[5] * C + g[8] * A, g[0] * t + g[6] * H, g[1] * y + g[7] * I, g[2] * C + g[8] * A
                    ], l = new M(l, d.side, d.level + 1, 2 * d.x + 1, 2 * d.y, m.fullpath), e.push(l));
                    d.x == s && z <= m.tileResolution || d.y == s && z <= m.tileResolution || (l = [g[0] * t + g[6] * H, g[1] * y + g[7] * I, g[2] * C + g[8] * A, g[3] * n + g[6] * x, g[4] * y + g[7] * I, g[5] * C + g[8] * A, g[6], g[7], g[8], g[9] * t + g[6] * H, g[10] * n + g[7] * x, g[11] * C + g[8] * A], l = new M(l, d.side, d.level + 1, 2 * d.x + 1, 2 * d.y + 1, m.fullpath), e.push(l));
                    d.y == s && z <= m.tileResolution || (l = [g[0] * n + g[9] * x, g[1] * y + g[10] * I, g[2] * C + g[11] * A, g[0] * t + g[6] * H, g[1] * y + g[7] * I, g[2] * C + g[8] * A, g[9] * t + g[6] * H, g[10] * n + g[7] * x, g[11] * C + g[8] * A, g[9], g[10], g[11]], l = new M(l, d.side, d.level + 1, 2 * d.x, 2 * d.y + 1, m.fullpath), e.push(l));
        ...
        E.style.left = 0;
        E.style.top = 0;
        E.setAttribute("id", "theSpace")
        console.log(E)

        var SCREEN = f.createElement("canvas")
        SCREEN.style.width = "1200px"
        SCREEN.style.height = "1200px";
        SCREEN.style.position = "absolute";
        SCREEN.style.left = 0;
        SCREEN.style.top = 0;
        SCREEN.style.boxSizing = "border-box";
        SCREEN.setAttribute("id", "theScreen")
        console.log(SCREEN)
       // E.classList.add("frontARGH")

        Z.appendChild(E);
        Z.appendChild(SCREEN);
        ...
        function drawLabel(xCoord, yCoord, distance) {
            ctx.font = 'bolder 28px serif';
            ctx.fillText(`${(distance/canvasToInchesFactor).toFixed(2)} in`, xCoord, yCoord);  
        }
        ...
        function mouseMove(e) {
            ...
            ctx.stroke();
            ctx.closePath();
             let distance = calculateDistance(stPoint.x, 
             stPoint.y, e.layerX, e.layerY)
             if (distance > 0 ) {
                ctx.fillStyle = "#FF0000"
                ctx.textAlign = "center"
                ctx.baseline = "bottom"
                drawLabel( ((e.layerX + stPoint.x)/2), ((e.layerY + stPoint.y)/2), distance)
             }
        ...
        function mouseUp(e) {
            ...
            // let distance = calculateDistance(stPoint.x, stPoint.y, endPoint.x, endPoint.y)
            // drawLabel(endPoint.x, endPoint.y, distance)
        }
        ...
        function drawLines() {
            linePoint.forEach(function(item) {
                ...
                ctx.stroke();
                ctx.closePath();
                let distance = calculateDistance(item['stPoint'].x, 
                item['stPoint'].y, item['endPoint'].x, item['endPoint'].y)
                ...
                ctx.baseline = "bottom"
                 drawLabel( 
                    (item['stPoint'].x + item['endPoint'].x)/2 , 
                    (item['endPoint'].y + item['stPoint'].y)/2, 
                    distance)
             }
            })
        ...
        this.init = function(G, d, Ra, L, k, g, e, s) {
            function z(a) {
                if (F) {
                    ...
                    for (n = 0; 6 > n; n++) 0 == m[n].width && (m[n] = a)
                }
            }
            ...
            if (c) {
                ...
                if (c.nodeCache)
                    for (G = 0; G < c.nodeCache.length; G++) a.deleteTexture(c.nodeCache[G].texture);
                a.deleteProgram(c);
                c = p
            }
            ea = p;
            var n, F = !1,
                x;
            if ("cubemap" == y)
                for (n = 0; 6 > n; n++) 0 < m[n].width ? (x === p && (x = m[n].width), x != m[n].width && console.log("Cube faces have inconsistent widths: " +
                    x + " vs. " + m[n].width)) : F = !0;
            ...
            if (!a && ("multires" == y && m.hasOwnProperty("fallbackPath") || "cubemap" == y) && ("WebkitAppearance" in
                ...
                var C = function() {
                    ...
                    F = !1;
                for (n = 0; 6 > n; n++) g = new Image, g.crossOrigin = qa.crossOrigin ? qa.crossOrigin : "anonymous", g.side = n, g.onload = k, g.onerror = C, g.src = "multires" == y ? L.replace("%s", M[n]) + "." + m.extension : m[n].src;
                z($)
            } else {
                ...
                G = Ma();
                xa = [];
                for (n = 0; 6 > n; n++) xa[n] = G.slice(12 * n, 12 * n + 12), G = Ma();
                G = 0;
                if ("equirectangular" == y) {
                    if (G = a.getParameter(a.MAX_TEXTURE_SIZE), Math.max(m.width / 2, m.height) > G) throw console.log("Error: The image is too big; it's " + m.width + "px wide, but this device's maximum supported size is " + 2 * G + "px."), {
                    ...
                    x + "px wide, but this device's maximum supported size is " + G + "px."), {
                    type: "webgl size error",
                    width: x,
                    maxWidth: G
                };
                ...
                d = sa;
                "cubemap" == y ? (x = a.TEXTURE_CUBE_MAP, d = bb) : "multires" == y && (d = ta);
                a.shaderSource(V, d);
                a.compileShader(V);
                c = a.createProgram();
                a.attachShader(c, U);
                ...
                L = a.getError();
                if (0 !== L) throw console.log("Error: Something went wrong with WebGL!", L), {
                    type: "webgl error"
                };
                e()
            }
        ...
        this.render = function(f, d, l, t) {
            ...
            if (a || "multires" != y && "cubemap" != y) {
                if ("multires" != y) l = 2 * Math.atan(Math.tan(0.5 * l) / (a.drawingBufferWidth / a.drawingBufferHeight)), l = 1 / Math.tan(0.5 * l), a.uniform1f(c.psi, d), a.uniform1f(c.theta, f), a.uniform1f(c.rot, g), a.uniform1f(c.f, l), !0 === ka && "equirectangular" == y && (a.bindTexture(a.TEXTURE_2D, c.texture), a.texImage2D(a.TEXTURE_2D, 0, a.RGB, a.RGB, a.UNSIGNED_BYTE, m)), a.drawArrays(a.TRIANGLES, 0, 6);
                    ...
                    for (e = 1; e < m.maxLevel && a.drawingBufferWidth > m.tileResolution * Math.pow(2, e - 1) * Math.tan(l / 2) * 0.707;) e++;
                    c.level = e;
                    e = [1, 0, 0, 0, 1, 0, 0, 0, 1];
                    e = pa(e, -g, "z");
                    e = pa(e, -f, "x");
                    ...
                    c.currentNodes = [];
                    e = "fbudlr".split("");
                    for (k = 0; 6 > k; k++) s = new M(xa[k], e[k], 1, 0, 0, m.fullpath), La(g, s, f, d, l);
                    ...
                    if (0 === ba.length)
                        ...
                        c.drawInProgress = !0;
                        a.clear(a.COLOR_BUFFER_BIT);
                        for (f = 0; f < c.currentNodes.length; f++) 1 < c.currentNodes[f].textureLoaded && (a.bindBuffer(a.ARRAY_BUFFER, fa), a.bufferData(a.ARRAY_BUFFER, new Float32Array(c.currentNodes[f].vertices), a.STATIC_DRAW), a.vertexAttribPointer(c.vertPosLocation, 3, a.FLOAT,
                            !1, 0, 0), a.bindBuffer(a.ARRAY_BUFFER, ya), a.vertexAttribPointer(c.texCoordLocation, 2, a.FLOAT, !1, 0, 0), a.bindTexture(a.TEXTURE_2D, c.currentNodes[f].texture), a.drawElements(a.TRIANGLES, 6, a.UNSIGNED_SHORT, 0));
                        c.drawInProgress = !1
                    }
                }
                ...
                for (k = $ / 2, t = {
                        ...
                        u: "translate3d(-" + (k + 2) + "px, -" + k + "px, " + (k + 2) + "px) rotateX(270deg)",// 270
...
window.pannellum = function(F, f, p) {
    function ja(t, l) {
            ...
            if (1 == a.getElementsByTagName("i").length) R();
                ...
                if ("cubemap" == b.type)
                    for (var v = 6, c = function() {
                        ...
                        }, a = 0; a < Q.length; a++) h = b.cubeMap[a], "null" == h ? (console.log("Will use background instead of missing cubemap face " + a), c()) : (b.basePath && !sa(h) && (h = b.basePath + h), Q[a].onload = c, Q[a].onerror = e, Q[a].src = I(h));
        ...
        function Z(a) {
            ...
            h.addEventListener("loadend", function() {
                ...
                if (-1 < c && !0 !== b.ignoreGPanoXMP) {
                    ...
                    null !== v && null !== c && null !== f && null !== g && null !== k && (0 > ca.indexOf("haov") && (b.haov = c / v * 360), 0 > ca.indexOf("vaov") && (b.vaov = g / f * 180), 0 > ca.indexOf("vOffset") && (b.vOffset = -180 * ((k + g / 2) / f - 0.5)), null !== l && 0 > ca.indexOf("northOffset") && (b.northOffset = l, !1 !== b.compass && (b.compass = !0)), null !== m && null !== e && (0 > ca.indexOf("horizonPitch") && (b.horizonPitch = m), 0 > ca.indexOf("horizonRoll") && (b.horizonRoll = e)))
                }
        ...
        function Ma(a) {
            b.minHfov === b.hfov ? ga.setHfov(za, 1E3) : (a = pa(a), ga.lookAt(a[0], a[1], b.minHfov, 1E3))
        }
        ...
        function wa(a) {
            la && (la = !1, 15 < Date.now() - O && (w.pitch = w.yaw = 0), J.classList.add("pnlm-grab"), J.classList.remove("pnlm-grabbing"), O = Date.now(), A("mouseup", a))
        }
        ...
        function Ha(a) {
            if (b.draggable && (a.preventDefault(), K && (O = Date.now()), la && K)) {
                var h = M(a.targetTouches[0]),
                    v = h.x,
                    c = h.y;
                2 == a.targetTouches.length && -1 != Ja && (a = M(a.targetTouches[1]), v += 0.5 * (a.x - h.x), c += 0.5 * (a.y - h.y),
                    h = Math.sqrt((h.x - a.x) * (h.x - a.x) + (h.y - a.y) * (h.y - a.y)), n(b.hfov + 0.1 * (Ja - h)), Ja = h);
        ...
        function c(a) {
            "touch" == a.pointerType && (ma.push(a.pointerId), Ca.push({
                clientX: a.clientX,
                clientY: a.clientY
        ...
        function V(a) {
            K && ("fullscreenonly" != b.mouseZoom || Da) && (a.preventDefault(), Ea(), O = Date.now(), a.wheelDeltaY ? (n(b.hfov - 0.05 * a.wheelDeltaY), w.hfov = 0 > a.wheelDelta ? 1 : -1) : a.wheelDelta ? (n(b.hfov - 0.05 * a.wheelDelta),
                w.hfov = 0 > a.wheelDelta ? 1 : -1) : a.detail && (n(b.hfov + 1.5 * a.detail), w.hfov = 0 < a.detail ? 1 : -1), D())
        }
        ...
        function ea(a, b) {
            var c = !1;
            switch (a) {
                case 109: // On Zoom Out
                case 189:
                case 17:
                case 173:
                    r[0] != b && (c = !0);
                    r[0] = b;
                    //canvasToInchesFactor *= 2
                    break;
                case 107: // On Zoom in
                case 187:
                case 16:
        ...
        function m() {
            if (K) {
                ...
                var f = (d - da) * b.hfov / 1700,
                    f = Math.min(f, 1);
                r[0] && !0 === b.keyboardZoom && (n(b.hfov + (0.8 * w.hfov + 0.5) * f), a = !0);
                r[1] && !0 === b.keyboardZoom && (n(b.hfov + (0.8 * w.hfov - 0.2) * f), a = !0);
                ...
                0 < f && !b.autoRotate && (a = 1 - b.friction, r[4] || r[5] || r[8] || r[9] || P.yaw || (b.yaw += w.yaw * f * a), r[2] || r[3] || r[6] || r[7] || P.pitch || (b.pitch += w.pitch * f * a), r[0] || r[1] || P.hfov || n(b.hfov + w.hfov * f * a));
                da = d;
                0 < f && (w.yaw =
        ...
        function fa() {
            ya();
            Va && clearTimeout(Va);
            if (la || !0 === Y) requestAnimationFrame(fa);
            else if (r[0] || r[1] || r[2] || r[3] || r[4] || r[5] || r[6] || r[7] || r[8] || r[9] || b.autoRotate || P.pitch || P.yaw || P.hfov || 0.01 < Math.abs(w.yaw) || 0.01 < Math.abs(w.pitch) || 0.01 < Math.abs(w.hfov)) m(),
                0 <= b.autoRotateInactivityDelay && X && Date.now() - O > b.autoRotateInactivityDelay && !b.autoRotate && (b.autoRotate = X, ga.lookAt(Ia, p, za, 3E3)), requestAnimationFrame(fa);
                ...
                0 < a ? Va = setTimeout(function() {
                    ...
                    D()
                }, a) : 0 <= b.autoRotateInactivityDelay && X && (b.autoRotate = X, ga.lookAt(Ia,
                    p, za, 3E3), D())
            }
        }
        ...
        function ya() {
            ...
            if (K) {
                ...
                var d = b.maxYaw - b.minYaw,
                    f = -180,
                    e = 180;
                360 > d && (f = b.minYaw + b.hfov / 2 + c, e = b.maxYaw - b.hfov / 2 - c, d < b.hfov && (f = e = (f + e) / 2), b.yaw = Math.max(f, Math.min(e,
                    b.yaw)));
                ...
                b.compass && (Ka.style.transform = "rotate(" + (-b.yaw -
                    b.northOffset) + "deg)", Ka.style.webkitTransform = "rotate(" + (-b.yaw - b.northOffset) + "deg)")
            }
        }
        ...
        function qa(a) {
            ...
            h = h.multiply(new aa(Math.sqrt(0.5),
                -Math.sqrt(0.5), 0, 0));
            c = F.orientation ? -F.orientation * Math.PI / 180 / 2 : 0;
            h = h.multiply(new aa(Math.cos(c), 0, -Math.sin(c), 0)).toEulerAngles();
            "number" == typeof Y && 10 > Y ? Y += 1 : 10 === Y ? ($a = h[2] / Math.PI * 180 + b.yaw, Y = !0, requestAnimationFrame(fa)) : (b.pitch = h[0] / Math.PI * 180, b.roll = -h[1] / Math.PI * 180, b.yaw = -h[2] / Math.PI * 180 + $a)
        }
        ...
        function ba() {
            try {
                ...
                b.backgroundColor !== p && (a.backgroundColor =
                    b.backgroundColor);
        ...
        function G(a) {
            ...
            if (a.video) {
                ...
                e.href = I(a.URL);
                if (a.attributes)
                    for (g in a.attributes) e.setAttribute(g,
                        a.attributes[g]);
                ...
                e.appendChild(c)
            } else a.sceneId && (c.onclick = c.ontouchend = function() {
                c.clicked || (c.clicked = !0, Sa(a.sceneId, a.targetPitch, a.targetYaw, a.targetHfov));
                return !1
            }, c.className += " pnlm-pointer", d.className += " pnlm-pointer"), N.appendChild(c);
            if (a.createTooltipFunc) a.createTooltipFunc(c, a.createTooltipArgs);
            else if (a.text || a.video || a.image) c.classList.add("pnlm-tooltip"), c.appendChild(d), d.style.width =
                d.scrollWidth - 20 + "px", d.style.marginLeft = -(d.scrollWidth - c.offsetWidth) / 2 + "px", d.style.marginTop = -d.scrollHeight - 12 + "px";
            a.clickHandlerFunc && (c.addEventListener("click", function(b) {
                a.clickHandlerFunc(b, a.clickHandlerArgs)
            }, "false"), c.className += " pnlm-pointer", d.className += " pnlm-pointer");
            a.div = c
        }
        ...
        function d() {
            ...
            }), b.hotSpots.forEach(G)) : b.hotSpots = [], Wa = !0, b.hotSpots.forEach(L))
        }
        ...
        function k(a) {
            ...
            for (c in l.default)
                if (l.default.hasOwnProperty(c))
                    if ("strings" == c)
                        for (d in l.default.strings) l.default.strings.hasOwnProperty(d) && (b.strings[d] = H(l.default.strings[d]));
            ...
            if (null !== a && "" !== a && l.scenes && l.scenes[a]) {
                ...
                for (c in f)
                    if (f.hasOwnProperty(c))
                        if ("strings" == c)
                            for (d in f.strings) f.strings.hasOwnProperty(d) && (b.strings[d] = H(f.strings[d]));
            ...
            for (c in l)
                if (l.hasOwnProperty(c))
                    if ("strings" == c)
                        for (d in l.strings) l.strings.hasOwnProperty(d) && (b.strings[d] = H(l.strings[d]));
            ...
            for (var e in b)
                if (b.hasOwnProperty(e)) switch (e) {
                    ...
                    case "author":
                        var g = H(b[e]);
                        b.authorURL &&
                            (g = f.createElement("a"), g.href = I(b.authorURL), g.target = "_blank", g.innerHTML = H(b[e]), g = g.outerHTML);
        ...
        function e() {
            if (K && !Oa)
                if (Da) f.exitFullscreen ? f.exitFullscreen() : f.mozCancelFullScreen ? f.mozCancelFullScreen() : f.webkitCancelFullScreen ? f.webkitCancelFullScreen() : f.msExitFullscreen && f.msExitFullscreen();
                    ...
                    t.requestFullscreen ?
                        t.requestFullscreen() : t.mozRequestFullScreen ? t.mozRequestFullScreen() : t.msRequestFullscreen ? t.msRequestFullscreen() : t.webkitRequestFullScreen()
                } catch (a) {}
        }

        function s(a) {
            f.fullscreenElement || f.fullscreen || f.mozFullScreen || f.webkitIsFullScreen || f.msFullscreenElement ? (u.fullscreen.classList.add("pnlm-fullscreen-toggle-button-active"), Da = !0) : (u.fullscreen.classList.remove("pnlm-fullscreen-toggle-button-active"), Da = !1);
        ...
        function x() {
            Oa && (q.load.box.style.display = "none", q.errorMsg.style.display = "none", Oa = !1, A("errorcleared"));
            K = !1;
            u.load.style.display = "none";
        ...
        function A(a) {
            if (a in T)
                for (var b = T[a].length; 0 < b; b--) T[a][T[a].length -
                    b
        ...
        W.className = "pnlm-dragfix"; //"pnlm"//
        W.id = "screenGrabLayer"
        console.log("WWW", W)
        J.appendChild(W);
        var ia = f.createElement("span");
        ia.className = "pnlm-about-msg";
        ia.innerHTML = '<a href="https://pannellum.org/" target="_blank">Pannellum</a> 2.5.2';
        J.appendChild(ia);
        ...
        /*
            Toggle Between Drawing and Dragging
        **/
        var drawBtn = document.getElementById("drawModeBtn")
        var panBtn = document.getElementById("panModeBtn")

        drawBtn.addEventListener("click", function() {
            // Remove the screen grab layer controls
            t.removeChild(J)

            // Change the styling of the 
            // Draw Btn selected styling
                drawBtn.style.backgroundColor = "red"
                drawBtn.style.color = "white"
                // Pan Btn unselected
                panBtn.style.background = "none"
        ...
        J.appendChild(Ka);
        l.firstScene ? k(l.firstScene) : l.default && l.default.firstScene ? k(l.default.firstScene) :
            k(null);
        g(!0);
        var ma = [],
            Ca = [];
        ...
        this.setNorthOffset = function(a) {
            b.northOffset = Math.min(360, Math.max(0,
                a));
            D();
            return this
        };
        ...
        this.addHotSpot = function(a, c) {
            if (c === p && b.scene === p) b.hotSpots.push(a);
                ...
                var d = c !== p ? c : b.scene;
                if (l.scenes.hasOwnProperty(d)) l.scenes[d].hasOwnProperty("hotSpots") || (l.scenes[d].hotSpots = [], d == b.scene && (b.hotSpots = l.scenes[d].hotSpots)),
                    l.scenes[d].hotSpots.push(a);
                else throw "Invalid scene ID!";
            }
            if (c === p || b.scene == c) G(a), K && L(a);
            return this
        };
        ...
        this.destroy = function() {
            B && B.destroy();
            Ta && (f.removeEventListener("mousemove", Ga, !1), f.removeEventListener("mouseup", wa, !1), t.removeEventListener("mozfullscreenchange", s, !1), t.removeEventListener("webkitfullscreenchange", s, !1), t.removeEventListener("msfullscreenchange", s, !1), t.removeEventListener("fullscreenchange", s, !1), F.removeEventListener("resize", ka,
                !1), F.removeEventListener("orientationchange", ka, !1), t.removeEventListener("keydown", $, !1), t.removeEventListener("keyup", xa, !1), t.removeEventListener("blur", S, !1), f.removeEventListener("mouseleave", wa, !1));
            t.innerHTML = "";
